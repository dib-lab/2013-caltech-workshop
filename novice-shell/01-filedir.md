...
You can download the example files and directories which Nelle is using,
...
Here,
we can see that `filesystem/users/nelle` contains seven [sub-directories](../../gloss.html#sub-directory).
...
Before we do this,
`pwd` shows us that we're in `filesystem/users/nelle`,
...
but if we run `pwd` after it, we can see that we are now in `/Users/SWC/Downloads/filesystem/users/nelle/data`.
If we run `ls` without arguments now,
it lists the contents of `/Users/SWC/Downloads/filesystem/users/nelle/data`,
...
> For example,
> if we are in `filesystem/users/nelle/data`,
> the command `ls ..` will give us a listing of `filesystem/users/nelle`.
